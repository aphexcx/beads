...
// ErrStorageTransactionsUnsupported reports that a tracker store does not
// expose the legacy storage transaction surface. Tracker features that use
// optional snapshot tables may degrade when this is returned.
var ErrStorageTransactionsUnsupported = errors.New("tracker store does not expose storage transactions")
...
// RunInStorageTransaction bridges fork-only optional snapshot features to the
// direct backend without widening the proxy-oriented Store contract.
func RunInStorageTransaction(ctx context.Context, store Store, commitMsg string, fn func(storage.Transaction) error) error {
	if direct, ok := store.(*directStore); ok {
		return direct.Storage.RunInTransaction(ctx, commitMsg, fn)
	}
	if legacy, ok := store.(storage.Storage); ok {
		return legacy.RunInTransaction(ctx, commitMsg, fn)
	}
	return ErrStorageTransactionsUnsupported
}

// LinearIssueSnapshotStoreFor resolves the fork's optional issue-snapshot
// capability through the direct-store adapter without making every adapter
// claim support for it.
func LinearIssueSnapshotStoreFor(store Store) (storage.LinearIssueSnapshotStore, bool) {
	if direct, ok := store.(*directStore); ok {
		if snapshots, ok := direct.Storage.(storage.LinearIssueSnapshotStore); ok {
			return snapshots, true
		}
		if dolt, ok := direct.Storage.(storage.DoltStorage); ok {
			snapshots, ok := storage.UnwrapStore(dolt).(storage.LinearIssueSnapshotStore)
			return snapshots, ok
		}
		return nil, false
	}
	snapshots, ok := store.(storage.LinearIssueSnapshotStore)
	return snapshots, ok
}

// LinearProjectSnapshotStoreFor is the Project-snapshot counterpart to
// LinearIssueSnapshotStoreFor.
func LinearProjectSnapshotStoreFor(store Store) (storage.LinearProjectSnapshotStore, bool) {
	if direct, ok := store.(*directStore); ok {
		if snapshots, ok := direct.Storage.(storage.LinearProjectSnapshotStore); ok {
			return snapshots, true
		}
		if dolt, ok := direct.Storage.(storage.DoltStorage); ok {
			snapshots, ok := storage.UnwrapStore(dolt).(storage.LinearProjectSnapshotStore)
			return snapshots, ok
		}
		return nil, false
	}
	snapshots, ok := store.(storage.LinearProjectSnapshotStore)
	return snapshots, ok
}

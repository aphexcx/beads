...
func cliCompatibleMigrationSQL(name, sqlText string) string {
	...
	case "0072_add_comment_external_ref.up.sql":
		...
		// migration guards each ALTER/CREATE INDEX with an INFORMATION_SCHEMA
		// probe (fork lineage already has the columns), but the CLI bundle's
...
func cliSubstituteAssumesWispTables(name string) bool {
	...
	case "0065_widen_wisp_comments_text.up.sql":
		...
		return true
	case "0027_add_wisp_comment_external_ref.up.sql":
		// Fork ignored-chain substitute (bd-5rs): bare ALTER/CREATE INDEX on
		// wisp_comments — same replay hazard class as 0065 above.
		return true
	default:
		return false
	}

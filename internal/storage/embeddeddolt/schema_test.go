...
func TestSchemaAfterInit(t *testing.T) {
	...
	// Linear sync snapshot tables (fork lineage): linear_label_snapshots via
	// main migration 0070, linear_issue_snapshots / linear_project_snapshots
	// via ignored migrations 0020/0021 (dolt-ignored, registered in 0071).
	for _, table := range []string{"linear_label_snapshots", "linear_issue_snapshots", "linear_project_snapshots"} {
		if _, err := db.ExecContext(ctx, "SELECT 1 FROM `"+table+"` LIMIT 0"); err != nil {
			t.Errorf("table %s not queryable after init: %v", table, err)
		}
	}

...
func ImportIssueCommentWithRefInTx(ctx context.Context, tx *sql.Tx, issueID, author, text, externalRef string, createdAt time.Time) (*types.Comment, error) {
	...
	createdAtText := FormatAuxTime(createdAt)
	// Same content derivation as InsertDerivedComment (external_ref is not in
	// the frozen digest column set): collapse onto an identical existing row
	// — stamping its external_ref — instead of inserting a duplicate, so
	// imports converge across clones (bd-ri8bd).
	var id string
	err := tx.QueryRowContext(ctx, fmt.Sprintf(`
		SELECT id FROM %s
		WHERE issue_id = ? AND author = ? AND text = ? AND created_at = ?
		ORDER BY id LIMIT 1`, commentTable),
		issueID, author, text, createdAtText).Scan(&id)
	inserted := false
	switch {
	case err == nil:
		if _, uerr := tx.ExecContext(ctx, fmt.Sprintf(
			`UPDATE %s SET external_ref = ? WHERE id = ? AND issue_id = ?`, commentTable), externalRef, id, issueID); uerr != nil {
			return nil, fmt.Errorf("stamp comment external_ref in %s: %w", commentTable, uerr)
		}
	case err == sql.ErrNoRows:
		digest := rowid.Digest([]sql.NullString{str(issueID), str(author), str(text), str(createdAtText)})
		id = rowid.New(commentTable, 0, digest)
		if _, ierr := tx.ExecContext(ctx, fmt.Sprintf(`
			INSERT INTO %s (id, issue_id, author, text, external_ref, created_at)
			VALUES (?, ?, ?, ?, ?, ?)
		`, commentTable), id, issueID, author, text, externalRef, createdAtText); ierr != nil {
			return nil, fmt.Errorf("add comment with ref to %s: %w", commentTable, ierr)
		}
		inserted = true
	default:
		return nil, fmt.Errorf("check comment existence in %s: %w", commentTable, err)
	}

	stored, perr := ParseAuxTime(createdAtText)
	if perr != nil {
		return nil, fmt.Errorf("add comment with ref to %s: %w", commentTable, perr)
	}
	// A genuinely new comment row is a bead mutation and journals like any
	// other structured comment; the collapsed path changed no content (only
	// the external_ref bookkeeping) and stays silent.
	if inserted {
		if jerr := RecordCommentEventInTx(ctx, tx, issueID, &EventComment{
			ID: id, Author: author, Text: text, CreatedAt: stored, Source: CommentSourceStructured,
		}); jerr != nil {
			return nil, jerr
		}

...
var beadDMLExemptions = map[string]string{
	...
	// Fork (linear comment sync): tracker-side mirror writes on existing
	// comment rows. external_ref stamping records where a comment landed
	// remotely (bookkeeping, no content change); a text update mirrors an
	// edit made ON the tracker, adopted verbatim during sync — both are
	// outside the create/update/close/delete/dep/label op vocabulary, and
	// the original import/create of each comment journaled its content.
	"UpdateCommentExternalRefInTx": "stamps tracker external_ref bookkeeping on an existing comment; no content change",
	"UpdateCommentTextInTx":        "adopts a tracker-side comment edit during Linear sync; outside the journal op vocabulary",

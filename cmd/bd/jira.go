...
func runJiraSync(cmd *cobra.Command, args []string) error {
	...
	if result.Stats.Pulled > 0 {
		fmt.Printf("✓ Pulled %d issues from Jira (%d created, %d updated locally)\n",
			result.Stats.Pulled, result.PullStats.Created, result.PullStats.Updated)
	}
	if result.Stats.Pushed > 0 {
		fmt.Printf("✓ Pushed %d issues to Jira (%d created, %d updated)\n",
...
func runJiraStatus(cmd *cobra.Command, args []string) error {
	...
	lastSync := tracker.LastSync(ctx, trackerStore, "jira")
